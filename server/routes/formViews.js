...
router.post("/formviews/formdata/insert", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const q = `
      ...
      RETURNING *
    `;
    const out = await client.query(q, [...vals, req.user.userId]); // using userId from verifyToken
...
// // Put near the top of this router file
// router.param('id', (req, res, next, id) => {
//   if (!/^\d+$/.test(id)) {
//     return res.status(400).json({ error: 'viewId must be an integer' });
//   }
...
// router.get('/:id(\\d+)', async (req, res) => {
//   const id = Number(req.params.id); // already numeric by regex
//   // ... your DB query using [id]
// });
