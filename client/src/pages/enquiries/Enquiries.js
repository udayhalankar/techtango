...
const Enquiries = () => {
  ...
  const handleNewEnquirySuccess = async () => {
    await fetchEnquiries();   // ensure joins/derived fields are fresh
    ...
    <Box p={3}>
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2} mt={3}>
        ...
        <Box display="flex" alignItems="center" gap={1}>
          ...
          />
          <ToggleButtonGroup value={view} exclusive onChange={(e, next) => next && setView(next)}>
      ...
      <Box mt={3} display="flex" justifyContent="flex-end" gap={2}>
        ...
        <Button disabled={currentPage * cardsPerPage >= filtered.length} onClick={() => setCurrentPage(currentPage + 1)}>Next</Button>
      </Box>
