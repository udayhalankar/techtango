...
export default function CustomApps() {
  ...
  const filtered = useMemo(
    () =>
      dashboards.filter((item) =>
        ...
        [item.app_name, item.app_slug, item.table_name, item.requirement]
  ...
  const getAppUrl = (item) => {
    const mode = String(item?.schema_json?.appMode || item?.schema?.appMode || "").toLowerCase();
    if (mode === "crud") {
              ...
              {filtered.map((item) => (
                <Grid item key={item.id} xs={12} sm={6} md={3}>
                  <Paper
                    elevation={0}
                    onClick={() =>
                      window.open(getAppUrl(item), "_blank")
                    }
                    sx={{
                      bgcolor: "#ffffff",
                      color: "#1f355d",
                  ...
                  >
                    <Box>
                      <Typography sx={{ fontWeight: 700, fontSize: 16, color: "#1a4fd8" }}>
                        {item.app_name || item.page_name || "Untitled Application"}
                      </Typography>
