...
const buildShellHeaderEditorConfig = (header = {}) => {
  ...
  const logoImageUrl = String(safeHeader.logoImageUrl || safeHeader.logoUrl || safeHeader.logo?.url || "").trim();
  return {
    type: "brandHeader",
...
export default function AIexperiencebuilderPage() {
  ...
  const loadColumns = async (tableName) => {
    ...
    if (!safeTable) return [];
    if (Array.isArray(columnsByTable[safeTable]) && columnsByTable[safeTable].length) {
  ...
  const loadTableRows = async (tableName) => {
    const safeTable = String(tableName || "").trim();
    if (!safeTable) return [];
    if (Array.isArray(tableRowsByTable[safeTable]) && tableRowsByTable[safeTable].length) {
      if (chromeTarget?.kind === "widget" && chromeTarget.widgetId) {
        updateGeneratedWidgetConfig(chromeTarget.widgetId, (currentConfig) => ({
          ...(currentConfig || {}),
    ...
    setGeneratedSpec((currentSpec) => {
      ...
      const nextWidgets = currentSpec.widgets.map((widget) => {
        ...
        return {
                ...
                _instanceId: widget._instanceId || `${widget.id}-${Date.now()}`
              };
      });
      return changed ? { ...currentSpec, widgets: nextWidgets } : currentSpec;
    });
  };
  ...
  const openWidgetOptions = (widget) => {
    ...
    setChromeWidgetConfig(widget.config || {});
    setChromeSectionPadding({
      top: 0,
      right: 0,
      bottom: 0,
    ...
    setChromeSectionTab(0);
    setChromeEditorOpen(true);
    if (widget.config?.tableName) {
      void loadColumns(widget.config.tableName);
      void loadTableRows(widget.config.tableName);
    }
  };
  ...
  if (field === "logoUrl" || field === "logoImageUrl") {
    nextPartial.logoUrl = String(value || "").trim();
    nextPartial.logoImageUrl = String(value || "").trim();
  }
  ...
  if (chromeTarget?.kind === "shell" && chromeTarget.slotKey === "header") {
    ...
    const logoValue = String(currentConfig.logoImageUrl || currentConfig.logoUrl || "").trim();

    const persistedHeader = {
      type: "brandHeader",
      visible: true,
      pageWidth: clampRange(currentConfig.pageWidth ?? 100, 20, 100),
      width: clampRange(currentConfig.width ?? 100, 20, 100),
      height: clampRange(currentConfig.height ?? 72, 40, 100),
      logoUrl: logoValue,
      logoImageUrl: logoValue,
      logoHeight: clampRange(currentConfig.logoHeight ?? 32, 16, 100),
      logoWidth: clampRange(currentConfig.logoWidth ?? 20, 5, 20),
      menuItems: parseHeaderItemsText(currentConfig.menuItemsText || currentConfig.menuItems || "", false),
      rightIcons: parseHeaderItemsText(currentConfig.rightIconsText || currentConfig.rightIcons || "", true),
    };
  ...
  const handleChromeImageUpload = (event) => {
    ...
    reader.onload = () => {
      ...
      if (!dataUrl) return;
      if (chromeTarget?.kind === "shell" && chromeTarget.slotKey === "header") {
        updateChromeWidgetField("logoImageUrl", dataUrl);
      } else {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const sanitizedPageSpec = {
        ...normalizedPageSpec,
        shell: {
          ...(normalizedPageSpec.shell || {}),
          showTopNavbar: false,
          topNavbar: {
            ...(normalizedPageSpec.shell?.topNavbar || {}),
            visible: false,
          },
        },
      };
      const pageSpec = validatePageSpec(sanitizedPageSpec);

      const withInstanceIds = {
        ...pageSpec,
        widgets: (pageSpec.widgets || []).map((w, i) => ({
          ...w,
          _instanceId: w._instanceId || `${w.id}-${i}-${Date.now()}`
        }))
      };

      setGeneratedSpec(withInstanceIds);
      setPreviewMode("html");
      setCreateOpen(false);
